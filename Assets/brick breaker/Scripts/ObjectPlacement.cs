...
public class ObjectPlacement : MonoBehaviour {
    ...
    public void PlaceNewObjectsOnTheScene() {
		...
		for (int i = 0; i < bricksXPosition.Length; i++) {
			int randBonusBall = Random.Range (0, 5); //Probability to spawn bonus ball
			int randStarBonus = Random.Range (0, 20); //Probability to spawn star
			if (randBonusBall == 1) {
